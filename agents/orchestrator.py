...
class TransitOrchestrator:
    ...
    def get_data_stats(self):
        with self.db.get_connection() as conn:
            ...
            trip_count = pd.read_sql("SELECT COUNT(*) as n FROM trips;", conn).iloc[0]['n']
            stats["Trip Records"] = f"{int(trip_count):,}"
            
            stop_count = pd.read_sql("SELECT COUNT(*) as n FROM stop_daily;", conn).iloc[0]['n']
            stats["Stop Records"] = f"{int(stop_count):,}"
            
            gis_count = pd.read_sql("SELECT COUNT(DISTINCT stop_id) as n FROM stops_reference;", conn).iloc[0]['n']
            ...
            routes = pd.read_sql("SELECT COUNT(DISTINCT service_rte_num) as n FROM trips;", conn).iloc[0]['n']
    ...
    def run(self, query: str) -> dict:
        ...
        chart = None
        if analysis_result.get("data") is not None and not analysis_result["data"].empty:
