...
# 100% coverage for combined/query_library.py
# ─────────────────────────────────────────────────────────────────────────────
# tests/test_query_library.py
# 100% coverage for updated_combined/query_library.py
# ─────────────────────────────────────────────────────────────────────────────
...
_sa.text          = lambda sql: sql          # pass-through so SQL is inspectable
...
def _engine_returning(rows, keys):
    """Fake engine whose execute() always returns the given rows & keys."""
    fake_result = MagicMock()
    fake_result.fetchall.return_value = rows
    fake_result.fetchone.return_value = rows[0] if rows else None
    fake_result.keys.return_value     = keys
    ...
    return fake_engine, fake_conn, fake_result


def _lib(rows, keys):
    engine, conn, result = _engine_returning(rows, keys)
    return TransitQueryLibrary(engine), conn, result


def _before_after_engine(before_row_tuple, after_row_tuple):
    """Engine that returns before_row on first fetchone(), after_row on second."""
    call_count = {"n": 0}
    fake_result = MagicMock()

    def _fetchone():
        r = before_row_tuple if call_count["n"] == 0 else after_row_tuple
        call_count["n"] += 1
        return r

    fake_result.fetchone.side_effect = _fetchone
    ...
    fake_engine = MagicMock()
    fake_engine.connect.return_value = fake_conn
    return TransitQueryLibrary(fake_engine)


# =============================================================================
# TestGetTopRoutesByRidership
# =============================================================================

class TestGetTopRoutesByRidership:
    _KEYS = ["route", "total_boardings", "total_alightings",
             "total_trips", "avg_load_factor", "crowded_trips"]

    def test_no_filters(self):
        lib, conn, _ = _lib([("40", 1000, 900, 50, 0.8, 5)], self._KEYS)
        df = lib.get_top_routes_by_ridership("2025-01-01", "2025-12-31")
        conn.execute.assert_called_once()
        assert list(df.columns) == self._KEYS

    def test_with_day_code_filter(self):
        lib, conn, _ = _lib([("40", 500, 450, 25, 0.7, 2)], self._KEYS)
        lib.get_top_routes_by_ridership("2025-01-01", "2025-12-31", day_code="WK")
        sql = conn.execute.call_args[0][0]
        assert "day_code" in sql

    def test_with_direction_filter(self):
        lib, conn, _ = _lib([("7", 300, 280, 15, 0.6, 1)], self._KEYS)
        lib.get_top_routes_by_ridership("2025-01-01", "2025-12-31", direction="I")
        sql = conn.execute.call_args[0][0]
        assert "inbd_outbd_cd" in sql

    def test_with_both_filters(self):
        lib, conn, _ = _lib([("E Line", 800, 750, 40, 0.9, 8)], self._KEYS)
        lib.get_top_routes_by_ridership("2025-01-01", "2025-12-31",
                                        day_code="WK", direction="I")
        sql = conn.execute.call_args[0][0]
        assert "day_code" in sql
        assert "inbd_outbd_cd" in sql
    ...
    def test_custom_top_n(self):
        lib, conn, _ = _lib([("40", 1000, 900, 50, 0.8, 5)], self._KEYS)
        lib.get_top_routes_by_ridership("2025-01-01", "2025-12-31", top_n=5)
        params = conn.execute.call_args[0][1]
        assert params["top_n"] == 5


# =============================================================================
# TestGetRouteRidershipTrend
# =============================================================================

class TestGetRouteRidershipTrend:
    _KEYS = ["period", "total_boardings", "total_alightings",
             "trip_count", "avg_load_factor", "crowded_trips"]

    def test_daily_aggregation(self):
        lib, conn, _ = _lib([("2025-01-01", 100, 90, 5, 0.7, 1)], self._KEYS)
        lib.get_route_ridership_trend("40", "2025-01-01", "2025-12-31", "daily")
        sql = conn.execute.call_args[0][0]
        assert "operation_date" in sql

    def test_weekly_aggregation(self):
        lib, conn, _ = _lib([("2025-01-06", 700, 650, 35, 0.8, 3)], self._KEYS)
        lib.get_route_ridership_trend("40", "2025-01-01", "2025-12-31", "weekly")
        sql = conn.execute.call_args[0][0]
        assert "week" in sql

    def test_monthly_aggregation(self):
        lib, conn, _ = _lib([("2025-01-01", 3000, 2800, 150, 0.85, 10)], self._KEYS)
        lib.get_route_ridership_trend("40", "2025-01-01", "2025-12-31", "monthly")
        sql = conn.execute.call_args[0][0]
        assert "month" in sql

    def test_default_aggregation_is_daily(self):
        lib, conn, _ = _lib([("2025-01-01", 100, 90, 5, 0.7, 1)], self._KEYS)
        lib.get_route_ridership_trend("40", "2025-01-01", "2025-12-31")
        sql = conn.execute.call_args[0][0]
        assert "operation_date" in sql
...
class TestGetBusiestStops:
    _KEYS = ["stop_id", "stop_nm", "total_boardings", "total_alightings",
             "days_with_data", "avg_departure_load", "total_trips"]

    def test_boardings_metric(self):
        lib, conn, _ = _lib([(1, "Main St", 500, 450, 30, 0.7, 100)], self._KEYS)
        lib.get_busiest_stops("2025-01-01", "2025-12-31", metric="boardings")
        assert "total_boardings" in conn.execute.call_args[0][0]

    def test_alightings_metric(self):
        lib, conn, _ = _lib([(1, "Main St", 500, 450, 30, 0.7, 100)], self._KEYS)
        lib.get_busiest_stops("2025-01-01", "2025-12-31", metric="alightings")
        assert "total_alightings" in conn.execute.call_args[0][0]

    def test_with_route_id_filter(self):
        lib, conn, _ = _lib([(1, "Pine St", 200, 180, 15, 0.6, 50)], self._KEYS)
        lib.get_busiest_stops("2025-01-01", "2025-12-31", route_id="40")
        assert conn.execute.call_args[0][1]["route_id"] == "40"

    def test_without_route_id(self):
        lib, conn, _ = _lib([(1, "Pine St", 200, 180, 15, 0.6, 50)], self._KEYS)
        lib.get_busiest_stops("2025-01-01", "2025-12-31")
        assert conn.execute.call_args[0][1]["route_id"] is None

    def test_custom_top_n(self):
        lib, conn, _ = _lib([(1, "Pine St", 200, 180, 15, 0.6, 50)], self._KEYS)
        lib.get_busiest_stops("2025-01-01", "2025-12-31", top_n=5)
        assert conn.execute.call_args[0][1]["top_n"] == 5


# =============================================================================
# TestAnalyzeServiceChangeImpact
# =============================================================================

class TestAnalyzeServiceChangeImpact:
    # Rows are TUPLES — query_library.py uses result[0], result[2], etc.
    # Columns: trips(0), days(1), avg_boardings(2), total_boardings(3),
    #          avg_load_factor(4), crowded_trips(5)

    def _lib(self, before_avg, after_avg,
             before_load=0.7, after_load=0.75):
        before = (10, 5, before_avg, before_avg * 10, before_load, 2)
        after  = (11, 5, after_avg,  after_avg  * 11, after_load,  1)
        return _before_after_engine(before, after)

    def test_basic_structure(self):
        result = self._lib(10.0, 12.0).analyze_service_change_impact("40", "2025-06-01", 30)
        for key in ("route_id", "change_date", "window_days",
                    "before_period", "after_period", "impact"):
            assert key in result
        assert result["route_id"] == "40"

    def test_before_period_has_end_date(self):
        result = self._lib(10.0, 12.0).analyze_service_change_impact("40", "2025-06-01", 30)
        assert "end_date" in result["before_period"]

    def test_decrease_direction_when_after_lower(self):
        result = self._lib(12.0, 10.0).analyze_service_change_impact("40", "2025-06-01", 30)
        assert result["impact"]["direction"] == "decrease"
    ...
    def test_significant_flag_true_when_change_over_5pct(self):
        result = self._lib(10.0, 16.0).analyze_service_change_impact("40", "2025-06-01", 30)
        assert result["impact"]["significant"] is True

    def test_significant_flag_false_when_change_under_5pct(self):
        result = self._lib(10.0, 10.3).analyze_service_change_impact("40", "2025-06-01", 30)
        assert result["impact"]["significant"] is False

    def test_zero_before_avg_boardings_no_division_error(self):
        result = self._lib(0.0, 12.0).analyze_service_change_impact("40", "2025-06-01", 30)
        assert result["impact"]["boardings_pct_change"] == 0

    def test_none_avg_boardings_treated_as_zero(self):
        # before_result[2] = None  → before_avg = 0, no crash
        before = (10, 5, None, None, None, 0)
        after  = (11, 5, 12.0, 132.0, 0.75, 1)
        lib = _before_after_engine(before, after)
        result = lib.analyze_service_change_impact("40", "2025-06-01", 30)
        # None propagates into the dict unchanged
        assert result["before_period"]["avg_boardings_per_trip"] is None

    def test_date_window_calculation(self):
        result = self._lib(10.0, 12.0).analyze_service_change_impact("40", "2025-06-01", 30)
        assert result["before_period"]["end_date"]   == "2025-05-31"
        assert result["after_period"]["start_date"]  == "2025-06-01"


# =============================================================================
# TestGetOvercrowdedRoutes
# =============================================================================

class TestGetOvercrowdedRoutes:
    _KEYS = ["route", "overcrowded_trips", "total_trips_count",
             "pct_overcrowded", "avg_max_load", "avg_threshold", "avg_excess_load"]

    def test_with_service_change_num(self):
        lib, conn, _ = _lib([("40", 5, 50, 10.0, 45.0, 40.0, 5.0)], self._KEYS)
        lib.get_overcrowded_routes("253")
        assert conn.execute.call_args[0][1]["service_change_num"] == "253"

    def test_with_time_period_filter(self):
        lib, conn, _ = _lib([("40", 5, 50, 10.0, 45.0, 40.0, 5.0)], self._KEYS)
        lib.get_overcrowded_routes("253", time_period="AM Peak")
        assert "time_period" in conn.execute.call_args[0][0]

    def test_without_time_period_not_in_params(self):
        lib, conn, _ = _lib([("40", 5, 50, 10.0, 45.0, 40.0, 5.0)], self._KEYS)
        lib.get_overcrowded_routes("253")
        assert "time_period" not in conn.execute.call_args[0][1]

    def test_raises_when_service_change_num_is_none(self):
        lib, _, _ = _lib([], self._KEYS)
...
class TestCompareRoutes:
    _KEYS = ["route", "total_boardings", "total_alightings", "total_trips",
             "avg_max_psngr_load", "crowded_trips", "pct_crowded"]

    def test_returns_dataframe_with_expected_columns(self):
        lib, _, _ = _lib([("40", 1000, 900, 50, 45.0, 5, 10.0)], self._KEYS)
        df = lib.compare_routes(["40", "7"], "2025-01-01", "2025-12-31")
        assert list(df.columns) == self._KEYS

    def test_date_params_passed(self):
        lib, conn, _ = _lib([("40", 1000, 900, 50, 45.0, 5, 10.0)], self._KEYS)
        lib.compare_routes(["40"], "2025-03-01", "2025-03-31")
        params = conn.execute.call_args[0][1]
...
# =============================================================================
# TestIdentifyDecliningRoutes
# =============================================================================

class TestIdentifyDecliningRoutes:
    _KEYS = ["route", "trip_count", "previous_avg_boardings", "recent_avg_boardings",
             "boardings_pct_change", "previous_load_factor", "recent_load_factor",
             "max_psngr_load_change"]

    def _lib_with_summary(self, rows):
        """Engine: first call → summary row; second call → main query rows."""
        summary_row = (date(2025, 1, 1), date(2025, 12, 31), 365, 50, 100000)

        summary_result = MagicMock()
        summary_result.fetchone.return_value = summary_row

        main_result = MagicMock()
        main_result.fetchall.return_value = rows
        main_result.keys.return_value = self._KEYS

        call_count = {"n": 0}
        fake_conn = MagicMock()

        def _execute(sql, params=None):
            r = summary_result if call_count["n"] == 0 else main_result
            call_count["n"] += 1
            return r
    ...
    def test_returns_dataframe(self):
        lib = self._lib_with_summary([("99", 200, 50.0, 30.0, -40.0, 0.8, 0.6, -0.2)])
        df = lib.identify_declining_routes()
        assert df.iloc[0]["route"] == "99"
    ...
    def test_custom_params_forwarded(self):
        captured = []
        lib = self._lib_with_summary([("99", 200, 50.0, 30.0, -40.0, 0.8, 0.6, -0.2)])
        fake_conn = lib.engine.connect.return_value

        summary_result = MagicMock()
        summary_result.fetchone.return_value = (
            date(2025, 1, 1), date(2025, 12, 31), 365, 50, 100000
        )
        main_result = MagicMock()
        main_result.fetchall.return_value = [("99", 200, 50.0, 30.0, -40.0, 0.8, 0.6, -0.2)]
        main_result.keys.return_value = self._KEYS
        ...
        fake_conn.execute.side_effect = _execute
        lib.identify_declining_routes(comparison_months=6, threshold_pct=-20.0, min_trips=50)
        assert captured[0]["threshold_pct"] == -20.0
        assert captured[0]["min_trips"]     == 50


# =============================================================================
# TestGetCrowdingByTimePeriod
# =============================================================================

class TestGetCrowdingByTimePeriod:
    _KEYS = ["time_period", "total_trips", "crowded_trips",
             "pct_crowded", "avg_max_psngr_load", "avg_boardings"]

    def test_without_route_filter(self):
        lib, conn, _ = _lib([("AM Peak", 100, 20, 20.0, 45.0, 30.0)], self._KEYS)
        lib.get_crowding_by_time_period(start_date="2025-01-01", end_date="2025-12-31")
        assert "service_rte_num" not in conn.execute.call_args[0][0]

    def test_with_route_filter_adds_where_clause(self):
        lib, conn, _ = _lib([("PM Peak", 80, 15, 18.75, 42.0, 28.0)], self._KEYS)
        lib.get_crowding_by_time_period(route_id="40",
                                        start_date="2025-01-01", end_date="2025-12-31")
        assert "service_rte_num" in conn.execute.call_args[0][0]


# =============================================================================
# TestGetRouteByDirection
# =============================================================================

class TestGetRouteByDirection:
    _KEYS = ["direction", "direction_label", "total_boardings",
             "total_trips", "avg_max_psngr_load"]

    def test_returns_dataframe(self):
        lib, _, _ = _lib(
            [("I", "Inbound", 500, 25, 40.0), ("O", "Outbound", 400, 20, 38.0)],
            self._KEYS,
        )
        df = lib.get_route_by_direction("40", "2025-01-01", "2025-12-31")
        assert len(df) == 2

    def test_route_id_passed_in_params(self):
        lib, conn, _ = _lib([("I", "Inbound", 500, 25, 40.0)], self._KEYS)
        lib.get_route_by_direction("40", "2025-01-01", "2025-12-31")
        assert conn.execute.call_args[0][1]["route_id"] == "40"


# =============================================================================
# TestGetRidershipByDayType
# =============================================================================

class TestGetRidershipByDayType:
    _KEYS = ["day_code", "day_type", "days", "total_trips",
             "total_boardings", "avg_boardings_per_trip", "avg_max_psngr_load"]

    def test_returns_dataframe(self):
        lib, _, _ = _lib(
            [("WK", "Weekday", 200, 1000, 50000, 50.0, 40.0),
             ("SA", "Saturday", 50, 200, 8000, 40.0, 35.0)],
            self._KEYS,
        )
        df = lib.get_ridership_by_day_type(start_date="2025-01-01", end_date="2025-12-31")
        assert len(df) == 2

    def test_with_route_id_adds_where_clause(self):
        lib, conn, _ = _lib([("WK", "Weekday", 200, 1000, 50000, 50.0, 40.0)], self._KEYS)
        lib.get_ridership_by_day_type(route_id="40",
                                      start_date="2025-01-01", end_date="2025-12-31")
        assert "service_rte_num" in conn.execute.call_args[0][0]

    def test_without_route_id_no_route_clause(self):
        lib, conn, _ = _lib([("WK", "Weekday", 200, 1000, 50000, 50.0, 40.0)], self._KEYS)
        lib.get_ridership_by_day_type(start_date="2025-01-01", end_date="2025-12-31")
        assert "service_rte_num" not in conn.execute.call_args[0][0]
...
class TestGetSummary:
    def _lib(self):
        row = (date(2025, 1, 1), date(2025, 12, 31), 365, 50, 100000)
        fake_result = MagicMock()
    ...
    def test_returns_correct_dict_structure(self):
        summary = self._lib().get_summary()
        assert summary["earliest_date"] == "2025-01-01"
        assert summary["latest_date"]   == "2025-12-31"
        assert summary["unique_routes"] == 50
        assert summary["total_trips"]   == 100000
